...
commands = ['help', 'weather', 'dn', 'а ты меня любишь?', 'фото', 'кто']
    ...
    for image in enumerate(images[:num]):
        for tag in image[1]:
            try:
                ...
                upl = urm['upload_url']
                req = requests.post(upl, files={'photo': open(imagePath, "rb")})
                req.close()
...
while (True):
    try:
        key = input('\tВведите ключ или 0, если ключ не используется:\t')
        if key != '0':
            ...
            session = vk.AuthSession(app_id=app, user_login=login, user_password=passw,
                                     scope='offline, messages, photos')
            api = vk.API(session)
        print('\n\tУспех\n')
        lf.write('Успех начала сессии\n\n')
        break
    except:
        print('Ошибка во введенных данных')
            ...
            if new_am > 0:
                ...
                try:
                    for m in messages:
                        ...
                        if comand == 'а ты меня любишь?':
                            send(to, user_id, ' Да, милая&#128154;&#128154;&#128154;')
                            continue
